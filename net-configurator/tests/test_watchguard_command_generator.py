...
def rule(network_peer_range, rule_filter):
    ...
    with patch.object(Rule, 'identifier', new_callable=lambda: 'test-rule-id'):
        return Rule(filter=rule_filter, sources=(network_peer_range,), destinations=(network_peer_range,), owners=('owner1', 'owner2'))
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            (
                NetworkPeer(ip_low='192.168.1.1'),
                NetworkPeer(ip_low='192.168.3.0/24'),
            ),
            (NetworkPeer(ip_low='192.168.2.1'),),
...
def test_add_rule_matrix(sources, destinations, services, owners, expected_policy_type_cmd, expected_policy_tag_cmd):
    ...
    with patch.object(Rule, 'identifier', new_callable=lambda: 'test-rule-id'), patch.object(RuleFilter, 'identifier', new_callable=lambda: 'test-filter-id'):
        rule_filter = RuleFilter(root=services)
        rule = Rule(filter=rule_filter, sources=sources, destinations=destinations, owners=owners)
...
def test_add_owner():
    ...
    expected_commands = ['config', 'policy', 'policy-tag owner1 color 0xc0c0c0', 'policy-tag owner2 color 0xc0c0c0', 'exit', 'exit']
