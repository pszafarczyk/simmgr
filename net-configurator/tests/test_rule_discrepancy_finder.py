...
from net_configurator.rule_discrepancy_finder import RuleDiscrepancyFinder
...
def create_ruleset() -> Callable[[str], set[Rule]]:
    ...
    rules: dict[str, dict[str, Any]] = {
        'a': {'sources': ({'ip_low': '10.1.3.173'},), 'destinations': ({'ip_low': '172.31.0.100'},), 'filter': ({'protocol': 'icmp'},)},
        'b': {'sources': ({'ip_low': '10.1.3.105'},), 'destinations': ({'ip_low': '0.0.0.0/0'},), 'filter': ({'protocol': 'icmp'},)},
        'c': {'sources': ({'ip_low': '10.0.0.0/8'},), 'destinations': ({'ip_low': '0.0.0.0/0'},), 'filter': ({'protocol': 'udp', 'port_low': 53},)},
    }
...
def test_discrepancy_empty_desired_give_empty_add(create_ruleset: Callable[[str], set[Rule]], existing_rule_symbols: str) -> None:
    ...
    finder = RuleDiscrepancyFinder(desired_rules=desired_rules, existing_rules=existing_rules)
    result = finder.get_rules_to_add()
    assert len(result) == 0
...
def test_discrepancy_empty_desired_give_existing_to_delete(create_ruleset: Callable[[str], set[Rule]], existing_rule_symbols: str) -> None:
    ...
    existing_rules = create_ruleset(existing_rule_symbols)
    finder = RuleDiscrepancyFinder(desired_rules=desired_rules, existing_rules=existing_rules)
    result = finder.get_rules_to_delete()
